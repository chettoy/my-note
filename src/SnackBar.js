...
class SnackBar {
  constructor() {
    ...
    this.__view = Utils.parseHTML('<div id="SnackBar" style="width:100%;height:3rem;position:fixed;left:0px;bottom:-3rem;z-index:999;background:rgba(244,67,102,0.9);color:white;font-size:1rem;"><span style="line-height:3rem;margin-left:1.25rem;"></span></div>')[0];
  ...
  dismiss(arg1, arg2) {
    ...
    if (view) {
      ...
      if (getComputedStyle(view)['display'] !== 'none' && view.getBoundingClientRect().left < document.documentElement.clientWidth) {
        Utils.animate(view, {bottom: -view.offersetHeight + 'px'}, 'fast', () => {
          _remove();
        });
      }else{
        _remove();
  ...
  show() {
    ...
    view.addEventListener('touchend', this._touchend = e => {
      if (parseInt(getComputedStyle(view)['left']) > document.documentElement.clientWidth / 3) {
        Utils.animate(view, {left: document.documentElement.clientWidth + 'px'}, 'normal', () => {
          this.dismiss();
        });
      }else{
        Utils.animate(view, {left: '0px'}, 'normal');
    ...
    Array.prototype.forEach.call(fabs, function(el, i){
      if (el.style.position === 'fixed' && document.documentElement.clientHeight - el.getBoundingClientRect().top - el.offsetHeight < view.offsetHeight && el.getBoundingClientRect().left < view.getBoundingClientRect().left + view.offsetWidth) {
        ...
        if (obj.bottom !== 'auto') {
          Utils.animate(el, {bottom: parseInt(obj.bottom) + view.offsetHeight + 'px'});
        }else if(obj.top !== 'auto'){
          Utils.animate(el, {top: parseInt(obj.top) - view.offsetHeight + 'px'});
        }else{
          Utils.animate(el, {bottom: view.offsetHeight + 'px'});
        }
        let has = false;
        for (const i in SnackBar.affectedFAB) {
